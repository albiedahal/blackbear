...
ModifiedMazarsDamage::validParams()
{
  ...
  params.addRequiredParam<std::vector<MaterialName>>("damage_models",
                                                     "Name of the damage models to combine with "
                                                     "tensile component of Mazars damage");
...
ModifiedMazarsDamage::initialSetup()
{
  ...
  {
    ScalarDamageBase * model = dynamic_cast<ScalarDamageBase *>(
    ...
    if (model)
      ...
      this->template paramError("damage_model",
                                "Damage Model " + _damage_models_names[i] +
                                    " is not compatible with MazarsDamage");
  }
}
...
ModifiedMazarsDamage::updateQpDamageIndex()
{
  ...
  {
    _damage_index[_qp] += 1-(1-alpha_t*tensile_damage)*(1-_damage_models[i]->getQpDamageIndex(_qp));
  }
  _damage_index[_qp]=std::max(_damage_index[_qp]+alpha_c*compressive_damage,0.0);
